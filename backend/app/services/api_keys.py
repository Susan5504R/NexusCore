...
logger = logging.getLogger("nexuscore.api_keys")
...
class ApiKeyService:
    ...
    async def generate_key(self, namespace: str, label: str = "") -> str:
        """Generates a new nx_core_ key, persists the hash, and returns the raw key."""
        raw_token = secrets.token_urlsafe(32)
        raw_key = f"nx_core_{raw_token}"
    ...
    async def resolve_key(self, raw_key: str) -> Optional[str]:
        ...
        if not raw_key.startswith("nx_core_"):
            return None
