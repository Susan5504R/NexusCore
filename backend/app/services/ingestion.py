...
logger = logging.getLogger("nexuscore.ingestion")
...
def _process_file(file_path: Path, repo_path: Path) -> List[Document]:
    """Reads a file, detects language, and chunks it contextually."""
    ...
    chunks = splitter.create_documents(
        [source_text], 
        metadatas=[{"path": str(rel_path).replace("\\", "/"), "language": language_str}]
    )
