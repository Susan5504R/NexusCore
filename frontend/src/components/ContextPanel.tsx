      ...
      <form onSubmit={handleAsk} className="space-y-3">
        <div className="grid grid-cols-1 sm:grid-cols-[1fr_auto] gap-3">
          <textarea
            ...
            placeholder="Ask anything about the ingested codebase, e.g. 'Where is the retry limit enforced?'"
            rows={2}
            onKeyDown={(e) => {
              if (e.key === "Enter" && (e.metaKey || e.ctrlKey)) handleAsk(e);
            }}
            className="w-full bg-base border border-surface rounded-lg px-3 py-2 text-sm text-text-main focus:outline-none focus:border-primary transition-colors placeholder:text-text-muted/50 resize-none"
          />
          <div className="flex sm:flex-col gap-3">
            <input
              ...
              className="w-full sm:w-36 bg-base border border-surface rounded-lg px-3 py-2 text-sm text-text-main focus:outline-none focus:border-primary transition-colors placeholder:text-text-muted/50"
            />
            <button
              type="submit"
              disabled={!prompt.trim() || isStreaming}
              className="flex items-center justify-center gap-2 px-4 py-2 rounded-lg font-semibold text-sm bg-primary text-base transition-all disabled:opacity-50 disabled:cursor-not-allowed hover:opacity-90"
            >
