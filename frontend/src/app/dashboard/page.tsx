...
export default function Dashboard() {
  ...
  // Apply default theme to HTML tag and gate the SaaS dashboard on a verified key.
  useEffect(() => {
    ...
    import("../../lib/systemClient").then(({ fetchSystemMode }) => {
      fetchSystemMode()
        .then(data => {
          ...
          setFeatures(feats);
          // In cloud mode there is no login: require a verified nx_core_ key.
          // Without one, send the user back to onboarding to generate/verify it.
          if (feats.api_keys && !getStoredSaasKey()) {
      ...
      <div className="max-w-6xl mx-auto space-y-8">
        ...
        <div className="flex justify-between items-center bg-surface p-6 rounded-2xl shadow-xl border border-primary/30">
          ...
          <div className="flex items-center gap-6">
            <a href="/onboarding" className="text-sm font-semibold text-primary hover:underline px-3 py-1 border border-primary rounded-md bg-primary/10">
              Onboarding Guide
            </a>
            {features.api_keys && (
